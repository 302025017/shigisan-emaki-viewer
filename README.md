...
2. **質問を入力**: テキスト入力欄に自由に質問を入力（表現が近い質問を自動で見つけて回答します）
3. **送信**: 「送信」ボタンをクリック
4. **応答を確認**: チャット形式でシステムからの応答が表示されます

補足: 回答が見つからない場合は、別の言い方で質問してみてください。
補足: テキスト入力式の技術解説は [TEXT_INPUT_TECH_GUIDE.md](TEXT_INPUT_TECH_GUIDE.md) を参照してください。
